...
# set up the 2x2 canvas
par(mfrow=c(2,2))

###################### top left graphic
# plot the line without the x-axis
plot(inf2$global_active_power, type="l", ylab="Global Active Power", xlab=NA,  
     xaxp=c(1, 2889, 2), xaxt="n", cex.lab=.7)
...
###################### top right graphic
plot(inf2$voltage, type="l", ylab="Voltage", xlab="Datetime", xaxp=c(1, 2889, 2),xaxt="n", cex.lab=.7)
axis(side=1, at=c(1, number_of_thursdays+1, total_records), labels=c("Thu","Fri","Sat"))
...
plot(inf2$sub_metering_1, type="l", ylab="Energy sub metering", xlab=NA,  
     xaxp=c(1, 2889, 2), col="black", xaxt="n", cex.lab=.7)
par(new=TRUE)
...
leg_text_width=strwidth("sub_metering_3)")*.85

# add legend
legend("topright", col=c("black","red","blue"), lty=1, cex=.7, bty="n", xjust=1, 
       legend=c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"), text.width=leg_text_width)

####################### botton right graphic
plot(inf2$global_reactive_power, type="l", ylab="Global_reactive_power", xlab="Datetime", xaxp=c(1, 2889, 2),xaxt="n")
axis(side=1, at=c(1, number_of_thursdays+1, total_records), labels=c("Thu","Fri","Sat"),cex.lab=.7)
